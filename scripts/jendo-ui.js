    ...
    if ("items" in jsonData) {
      ...
      if (!jendo.isNullOrWhiteSpace(jsonData.link)) {
        ...
        userItem.click(function() {
          document.dispatchEvent(new CustomEvent("navbar.navigate", {detail:jsonData.args}));
        });
      }
    }
  }
...
function JendoDatepicker(element, options) {
  ...
  if (jendo.isNull(this.dpOptions)) {
    this.dpOptions = {weekdayStart:1, format:"yyyy-MM-dd", viewMode:"days", showButton:false};
...
function JendoGridView(element) {
  ...
  if (jendo.isNull(this._GVData)) {
    this._GVData = {filterable:{enabled:false, mode:"row"}, columns:[], records:[], sortData:[], filterData:[], allowPaging:true, pageSize:10, pageNo:1, pager:{numeric:true, numericCount:5, prevNext:true, firstLast:true, goToPage:true, pageSize:true, pageSizeList:[], info:true}, show:{rowNo:false}};
...
JendoGridView.prototype.appendGRows = function(tBody, gColumns, gRecords, gvData) {
  var countViewRows = gRecords.length;
  if (this._GVData.allowPaging) {
    if (countViewRows > this._GVData.pageSize) {
      countViewRows = this._GVData.pageSize;
    }
  }
  for (var i = 0;i < countViewRows;i++) {
    var gRow = gRecords[i];
...
JendoGridView.prototype.appendGPaging = function(tBody, gColumns, gRecords, gvData) {
  if (gvData.allowPaging) {
    ...
    var countPage = Math.round(countPageTmp, 0);
    if (countPageTmp > countPage) {
      countPage += 1;
    }
    td.attr("colspan", countColumns);
    var pager = gvData.pager;
    if (pager.firstLast) {
      var btnFirst = td.appendElementButton().addClass("jd-pager-btn");
    ...
    if (pager.prevNext) {
      var btnPrev = td.appendElementButton().addClass("jd-pager-btn");
    ...
    if (pager.numeric) {
      var numericCount = pager.numericCount > countPage ? countPage : pager.numericCount;
      for (var i = 1;i <= numericCount;i++) {
        var btnNumeric = td.appendElementButton(i).addClass("jd-pager-btn")
      }
    }
    if (pager.prevNext) {
      var btnNext = td.appendElementButton().addClass("jd-pager-btn");
      btnNext.appendElement("i").attr("class", "fa fa-angle-right").attr("aria-hidden", true);
      if (gvData.pageNo == countPage) {
        btnNext.attr("disabled", "");
      }
    }
    if (pager.firstLast) {
      var btnLast = td.appendElementButton().addClass("jd-pager-btn");
    ...
    if (pager.goToPage) {
      ...
      pnlGTP.appendElement("span").text("of " + countPage);
      pnlGTP.appendElementButton("Go").addClass("jd-pager-btn").click(function() {
        console.log("GO", textGTP.val());
      });
    }
    if (pager.pageSize) {
      ...
      if (!jendo.isNull(gCol.commands)) {
        for (var c = 0;c < gCol.commands.length;c++) {
          ...
          if (!jendo.isNull(command.click)) {
            var commandValue = {rowIndex:rowIndex, colIndex:f};
